function [X, A, B, C,residual] = GRAD_TENSOR(Y,tmax,gamma,eps_1,eps_2,A_0,B_0,C_0,max_iter,itrs2,decay)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  % Size of the tensor Y and the core tensor X
    M1 = size(A_0,2); M2 = size(B_0,2); M3 = size(C_0,2);
  ...
  % Cycle that stops when quantity is less than eps_2
    ...
    while( (residual(iter+1) > eps_2)  && (iter< max_iter))
        %TOMP iteration
          [E,m1,m2,m3,r] = efficientTOMP2(A_0,B_0,C_0,Y,tmax,eps_2,itrs2);
          X = sptensor([],[],[M1,M2,M3]);
          E = sptensor(E);
          X(m1,m2,m3) = E;
          if iter+1==1
              residual(1)=norm(Y-ttm(X,{A_0,B_0,C_0}));
          end
  % We compute the gradient for A

          z_A  = ttm(X,{B_0,C_0},[2,3]); zr_A = unfold(tensor(z_A),1);
          Grad_A = ( Y_1 - A_0 * double(zr_A) ) * pinv( double(zr_A) );
   

  % We compute the gradient for B

          z_B  = ttm(X,{A_0,C_0},[1,3]); zr_B = unfold(tensor(z_B),2);
    
          Grad_B = ( Y_2 - B_0 * double(zr_B) ) * pinv( double(zr_B) );

  % We compute the gradient for C

          z_C = ttm(X,{A_0,B_0},[1,2]); zr_C = unfold(tensor(z_C),3);
...
%Gradient descent iterations for B
 ...
 %Gradient descentiterations for C
          ...
          residual(iter+1) = norm( Y - ttm(X,{A_0,B_0,C_0}) );
